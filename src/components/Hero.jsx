...
const PRIMARY_COLOR = 0x8a2be2; // BlueViolet
const SECONDARY_COLOR = 0x00ffff; // Cyan
...
const Hero = () => {
    ...
    return (
        <section id="hero" className="min-h-screen bg-gray-950 text-white relative overflow-hidden">
            {/* 3D Canvas */}
            <div 
                ref={canvasRef} 
            ...
            <div className="absolute inset-0">
                <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-purple-600/20 rounded-full mix-blend-soft-light filter blur-3xl animate-pulse"></div>
                <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-cyan-600/20 rounded-full mix-blend-soft-light filter blur-3xl animate-pulse delay-1000"></div>
                <div className="absolute top-1/2 left-1/2 w-96 h-96 bg-blue-600/10 rounded-full mix-blend-soft-light filter blur-3xl animate-pulse delay-500"></div>
            </div>

            {/* Content */}
            <div className="relative z-10 flex flex-col items-center justify-center min-h-screen px-4 lg:px-12 text-center pt-20 pb-16">
                
                {/* Badge */}
                <div className={`inline-flex items-center gap-2 px-6 py-2 rounded-full border border-indigo-700/50 bg-indigo-900/20 mb-8 backdrop-blur-sm shadow-xl 
                    ...
                    <span className="text-sm font-medium text-cyan-400 tracking-wider">Innovating Digital Solutions</span>
                </div>

                {/* Headline */}
                <h1 className={`text-4xl sm:text-6xl lg:text-7xl font-bold mb-6 transition-all duration-1000 delay-300 
                    ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"}`}>
                    <span className="bg-gradient-to-r from-cyan-400 via-indigo-400 to-purple-400 bg-clip-text text-transparent">
                        Building Intelligent
                    </span>
                    <br />
                    <span className="bg-gradient-to-r from-purple-400 via-indigo-500 to-cyan-500 bg-clip-text text-transparent">
                        Digital Experiences
                    </span>
                </h1>

                {/* Subtext */}
                <p className={`text-lg sm:text-xl lg:text-2xl text-gray-300 max-w-3xl mx-auto mb-8 leading-relaxed transition-all duration-1000 delay-500 
                    ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"}`}>
                ...
                <div className={`flex flex-col sm:flex-row gap-4 justify-center items-center mb-12 transition-all duration-1000 delay-700 
                    ...
                    <button className="btn btn-primary btn-lg group relative overflow-hidden border-0 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700">
                    ...
                    <button className="btn btn-outline btn-lg group border-gray-600 hover:border-cyan-400 text-gray-300 hover:text-white">
                        <span className="group-hover:text-white transition-colors duration-300">Our Services</span>
                        <div className="w-0 group-hover:w-5 h-0.5 bg-cyan-400 transition-all duration-300 ml-2"></div>
                    </button>
                </div>

                {/* Stats */}
                <div className={`grid grid-cols-2 md:grid-cols-4 gap-8 p-6 max-w-2xl w-full mx-auto rounded-xl border border-gray-700/50 backdrop-blur-lg bg-gray-800/20 shadow-2xl 
                    transition-all duration-1000 delay-900 ${isVisible ? "opacity-100 scale-100" : "opacity-0 scale-95"}`}>
                    {stats.map((stat, index) => (
                        <div key={index} className="text-center group cursor-pointer p-2 hover:bg-gray-800/50 rounded-lg transition-colors duration-300">
                            <div className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent group-hover:scale-110 transition-transform duration-300">
                                {stat.number}
                            </div>
